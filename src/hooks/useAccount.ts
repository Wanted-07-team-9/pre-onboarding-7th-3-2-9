...
export const useAccountInfo = (id:number, user_id:number) : [isLoading : boolean,isError:boolean,accountData:accountType,userData:userType]=> {

  const queryData = useQueries({
    queries: [
      { queryKey: ['userList',user_id], queryFn: ()=>userAPI.getUserById(user_id), staleTime: 1000 * 60*60},
      {
